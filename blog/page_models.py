...
checkup = Checkup(providers=["wagtail_page", "request", "google_page_speed"])
...
###############################################################################
# Abstract pages providing pre-built common safebear_cms functionality, suitable for subclassing.
...
class CoderedEventPage(CoderedWebPage, BaseEvent):
    ...
    def seo_struct_event_dict(self) -> dict:
        ...
        if self.seo_image:
            sd_dict.update(
                {"image": get_struct_data_images(self.get_site(), self.seo_image)}
            )
        ...
        for event in qs:
            ...
            for occurrence in occurrences:
                ...
                event_data = {
                    ...
                    "end": (
                        local_end.strftime("%Y-%m-%dT%H:%M:%S%z") if local_end else ""
                    ),
                    "description": "",
                }
                if event.url:
...
class CoderedFormMixin(models.Model):
    ...
    def process_data(self, form, request):
        ...
        for key, val in form.cleaned_data.items():
            if type(val) is InMemoryUploadedFile or type(val) is TemporaryUploadedFile:
...
class CoderedLocationPage(CoderedWebPage):
    ...
    safebear_cms = models.TextField(blank=True, verbose_name=_("safebear_cms"))
